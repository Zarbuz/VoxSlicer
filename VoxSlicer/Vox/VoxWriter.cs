...
namespace VoxSlicer.Vox
{
    public class VoxWriter : VoxParser
    {
        ...
        public bool WriteModel(string absolutePath, Schematic schematic, int direction, int volumeSize = 126) 
        {
        ...
        private void GetFirstBlockForEachRegion()
        {
            ...
            for (int i = 0; i < _countSize; i++)
            {
                int x = (i / (min * _height) * _volumeSize);
                int y = (((i / min) % _height) * _volumeSize);
                int z = ((i % min) * _volumeSize);
        ...
        {
            int worldPosX = _firstBlockInEachRegion[index].X - (_length / 2) * _volumeSize;
            int worldPosZ = _firstBlockInEachRegion[index].Z - (_width / 2) * _volumeSize;
            int worldPosY = _firstBlockInEachRegion[index].Y + _volumeSize;
        ...
        private void WriteSizeChunk(BinaryWriter writer)
        {
            ...
            writer.Write(_volumeSize); //Width
            writer.Write(_volumeSize); //Height
            writer.Write(_volumeSize); //Depth
        }
        ...
        private void WriteXyziChunk(BinaryWriter writer, int index)
        {
            ...
            {
                writer.Write((byte)(block.X % _volumeSize));
                writer.Write((byte)(block.Y % _volumeSize));
                writer.Write((byte)(block.Z % _volumeSize));
